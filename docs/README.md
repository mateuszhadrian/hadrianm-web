...
| `analiza-podstrona-dla-kogo.md` | **Plan wykonawczy** przeniesienia sekcji „Dla kogo" na podstronę `/dla-kogo/` (EN: `/en/who-its-for/`): strona główna zostaje ze statyczną zajawką (pierwsza klatka sceny bez licznika, przycisk `MoreLink` wg `docs/design/export-button-dla-kogo/`), podstrona = pełna animacja 1:1 na Lenisie (inaczej niż `/realizacje` — tam scroll natywny), BackButton + Footer wg wzorca `/realizacje`, CTA rozdziału 03 jako placeholder `#`, navbar „Dla kogo" → podstrona |
