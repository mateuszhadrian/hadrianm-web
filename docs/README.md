...
| `analiza-podstrona-oferta-hub.md` | **Plan wykonawczy** podstrony-huba `/oferta/` (EN: `/en/services/`) wg referencji `docs/design/export-oferta-hub/`: czwarty wariant `hub` w `Services.astro` (wstęp + karty Pakiety/Proces, `min-height: 100svh`), buttony współdzielone z zajawką (nowy `SplitCta.astro` wydzielony z `OfertaButtons`), navbar „Oferta" → podstrona (koniec ery kotwic w menu), CTA `dk-cta` na `/dla-kogo/` → `/oferta/`, scroll w pełni natywny, reveal `revealOnce` — **WDRAŻANE** (2026-07-19) |
| `visual-corrections-part1.md` | **Plan** poprawek wizualnych część 1 (2026-07-18): 8 punktów — unifikacja CTA (Dla kogo → SolidButton, Oferta → animowany wzorzec banera Kontakt), teksty i parametr skali kafelków Realizacje, wcześniejszy reveal Oferty, wspólny token odstępów sekcji, chrome polityki prywatności, preloady fontów + mikro-fade przeciw FOUC — **ZAAKCEPTOWANY** (2026-07-18; pkt 7: tylko navbar+footer, bez ambientu) z samowystarczalnymi instrukcjami-promptami per punkt; wdrażanie: osobne sesje, kolejność 1–5+7 → 6 → 8, baseline'y raz na końcu |
