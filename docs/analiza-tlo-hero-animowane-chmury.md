...
> ℹ️ **AKTUALIZACJA (2026-07-13): warianty kolorystyczne red/blue.** Komponent przyjmuje prop `variant: "red" | "blue"` — mechanika (gradienty, animacje, kropki, winieta) jest wspólna, różnią się wyłącznie kolory. Jedyne źródło prawdy o kolorach: `src/components/backgrounds/ambient-palette.ts` (pary oklch + rgb-fallback per plama; rgb przeliczone z oklch przez Chromium/canvas, nie dobierane na oko). Paletę konsumują: komponent (zmienne CSS przez `define:vars`; wybór oklch↔rgb robi `@supports` przez zmienne-aliasy `--c-*`, bo `var()` z niewspieranym oklch nie cofa się do wcześniejszej deklaracji) oraz `scripts/capture-ambient-bg.mjs`, który generuje statyczne tekstury mobile `public/ambient-bg-mobile-{red,blue}.webp` (render deterministyczny — regeneracja red wyszła bajt-w-bajt zgodna z historycznym plikiem). Przypisanie: warstwa `hero` = red, warstwa `work` = blue (`Home.astro`). Sekcje od About w dół mają własne, nieprzezroczyste tła — ambient jest tam niewidoczny, więc blue realnie kończy się wraz z Realizacjami. Perf mobile: warstwa `is-inactive` NIE pobiera swojej tekstury (`background-image: none`) — od rozdziału wariantów to dwa osobne pliki i eager fetch obu psuł budżet LCP; obraz dociąga się przy pre-warmie crossfade'u (~1 viewport przed fade-in).
