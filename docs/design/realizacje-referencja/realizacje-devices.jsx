...
function rzGray(l) { return `hsl(240 5% ${l.toFixed(1)}%)`; }
...
function RzReflection() {
  return (
    <div style={{
      ...
      background: 'linear-gradient(112deg, rgba(255,255,255,0.10) 0%, rgba(255,255,255,0.035) 26%, rgba(255,255,255,0) 45%)',
    }}></div>
  );
}
  ...
  return (
    <div style={{ width, position: 'relative' }}>
      <div style={{
        ...
        <div style={{ borderRadius: 8, overflow: 'hidden', position: 'relative', background: '#000', height: screenH }}>
          <RzScreenImg src={src} alt={alt}></RzScreenImg>
          <RzReflection></RzReflection>
        </div>
      </div>
      <div style={{
        position: 'relative', height: Math.round(width * 0.0216), width: '114%', marginLeft: '-7%',
        ...
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: width * 0.16, height: 7, background: 'rgba(0,0,0,0.45)', borderRadius: '0 0 10px 10px' }}></div>
      </div>
    </div>
  );
}
...
function RzPhone({ t, width = 224, chassis = 88, src, alt }) {
  ...
  return (
    <div style={{
      width, position: 'relative', boxSizing: 'border-box',
      background: `linear-gradient(180deg, ${rzGray(4 + chassis * 0.30)} 0%, ${rzGray(1.5 + chassis * 0.158)} 60%, ${rzGray(1 + chassis * 0.118)} 100%)`,
      borderRadius: outerR,
      padding: bezel,
      boxShadow: `inset 0 0 0 1px rgba(255,255,255,0.10), 0 60px 120px -30px rgba(0,0,0,0.85), 0 30px 100px -20px ${t.glow}`,
    }}>
      <div style={{ borderRadius: outerR - bezel, overflow: 'hidden', position: 'relative', background: '#000', height: screenH }}>
        <RzScreenImg src={src} alt={alt}></RzScreenImg>
        <div style={{ position: 'absolute', top: 6, left: '50%', transform: 'translateX(-50%)', width: 5, height: 5, borderRadius: 99, background: '#0a0a0a', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.06)', zIndex: 2 }}></div>
        <RzReflection></RzReflection>
      </div>
    </div>
  );
